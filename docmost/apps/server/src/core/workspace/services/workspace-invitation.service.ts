...
import { executeTx } from '@Pyjri/db/utils';
import {
  Group,
  User,
  WorkspaceInvitation,
...
import InvitationEmail from '@Pyjri/transactional/emails/invitation-email';
import { hashPassword } from '../../../common/helpers';
import { GroupUserRepo } from '@Pyjri/db/repos/group/group-user.repo';
import InvitationAcceptedEmail from '@Pyjri/transactional/emails/invitation-accepted-email';
...
export class WorkspaceInvitationService {
  ...
  async acceptInvitation(dto: AcceptInviteDto, workspaceId: string) {
    ...
    if (invitedByUser) {
      ...
      await this.mailService.sendToQueue({
        ...
        subject: `${newUser.name} has accepted your Pyjri invite`,
  ...
  async sendInvitationMail(
    ...
    await this.mailService.sendToQueue({
      to: inviteeEmail,
      subject: `${invitedByName} invited you to Pyjri`,
      template: emailTemplate,
    });
  }
}
